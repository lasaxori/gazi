...
""".format(str(ort_net)[:4],str(float(300-2*ort_net)/3)[:4]))
        raw_input("[ent]")
        exit()
    else:
        print("""
...
""".format(str((float(hdf*10 - ilkd*4)*10/6 - quiz_t*3/14 - float(vize_t*7)/3) / (float(kalan_v*7)/3 + float(kalan_q*3)/14))[:4],str(ort_net)[:4]))
        raw_input("[ent]")
        exit()
exit()
